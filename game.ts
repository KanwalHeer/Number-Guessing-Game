...
class Game {
    ...
    do {
      ...
      switch (answer.select) {
        ...
        case "Exit":
          exit = true;
          console.log(
            chalk.yellow(`You exited the game, Thank you and good bye!`)
          );
          break;
        default:
          break;
  ...
  private async playGame(): Promise<void> {
    ...
    if (this.userNumber === computerGuessNumber) {
      console.log(
        chalk.green(
          `Computer guess number is: ${computerGuessNumber} and your guess number is ${this.userNumber} ,congratulations you have won the game`
        )
      );
    } else if (this.userNumber > computerGuessNumber && this.userNumber < 10) {
      console.log(
        chalk.redBright(
          `Computer guess number is: ${computerGuessNumber} and your guess number is ${this.userNumber} your guess number is greater,you have lost the game`
        )
      );
    } else if (this.userNumber < computerGuessNumber && this.userNumber > 0) {
      console.log(
        chalk.redBright(
          `Computer guess number is: ${computerGuessNumber} and your guess number is ${this.userNumber}  yuor guess number is lower,you have lost the game`
        )
      );
    } else if (this.userNumber > 10 || this.userNumber < 0) {
      console.log(
        chalk.blueBright(`play again and choose number between 1 t0 10`)
      );
    }
  }
}
